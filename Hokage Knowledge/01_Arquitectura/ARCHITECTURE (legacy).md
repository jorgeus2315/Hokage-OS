> ⚠️ **SUPERADO** — Este documento describe una versión anterior del sistema (5 agentes con roles que ya no existen — Diseñador, Tesorero, Vendedor —, una tabla `agent_tasks` que nunca se implementó, `businesses` en vez de `ventures`). Sin tocar desde 2026-08-02. **[[Núcleo - Arquitectura del Core]] tiene prioridad donde contradiga a este documento.**
>
> Partes que siguen vivas y están ratificadas explícitamente en la fuente de verdad actual: §12 "Lo que NUNCA cambia" (ver [[Núcleo - Arquitectura del Core]]). El resto se conserva aquí solo como referencia histórica — no se debe diseñar ni construir contra este documento.
>
> Fuente: `ARCHITECTURE.md` (raíz del repo), verbatim.
...
## Notas relacionadas (mapeo a la fuente de verdad actual)

- [[Núcleo - Arquitectura del Core]] — §1, §12 (parcialmente ratificados)
- [[VISION]] — visión de producto que este documento intentaba servir
- [[Runtime, Scheduler y Event Bus]] — §5, §6, §7 actualizados (work_items, scheduler de 8 etapas sigue vigente en espíritu, ver spec actual)
- [[Agentes - Modelo y Decisión]] — §4 (roles Diseñador/Tesorero/Vendedor ya no existen, ver nota actual)
- [[Plugin System - Arquitectura Completa]] — §9 (Tools), contrato actualizado sin `ZodSchema`/`BaseTool`
- [[Economía]] — §13
- [[Modelo Multi-Venture]] — `businesses` → `ventures` en el modelo actual
- [[Recetas - Añadir Negocio]] — §12, §15 (ciclo de vida de negocio)
- [[Frontend World Engine]] · [[Frontend - Decisiones v2]] — §3, §8, §16
